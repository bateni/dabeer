...
%% For documentation of itemize keys:
...
\startbuffer[itemlistshort]
\startitem شرق \stopitem
\startitem غرب \stopitem
\startitem شمال \stopitem
\startitem جنوب \stopitem
\stopbuffer
...
باشید که در این‌جا قابلیت شکستن خطوط در میان یک فقره وجود ندارد.

\startitemize[ع,horizontal,four]
\getbuffer[itemlist]
\stopitemize

در ادامه از شماره‌گذاری‌های دیگر ویژه فارسی استفاده می‌کنیم.  ابتدا با
استفاده از کلید \type{الف} از الفبای فارسی بهره می‌گیریم و نشانه‌ها را
میان پرانتز قرار داده و وسط‌چین می‌نماییم.

\startitemize[الف,horizontal,four][left=(,right=),stopper=,itemalign=middle,distance=1ex]
\getbuffer[itemlist]
\getbuffer[itemlistshort]
\getbuffer[itemlist]
\getbuffer[itemlistshort]
\stopitemize

سپس با کلید \type{ابجد} که معادل \type{ا} می‌باشد از ترتیب ابجد عربی
استفاده می‌کنیم.  این‌جا تنها پس از حروف پرانتز بسته گذاشته و نشانه را
چپ‌چین نموده‌ایم.  توجه داشته باشید که کلیدهای \type{آ} و \type{ا}
متفاوت‌اند.

\startitemize[ابجد,horizontal,four][right=),stopper=,itemalign=left,distance=1ex]
\getbuffer[itemlist]
\getbuffer[itemlistshort]
\getbuffer[itemlist]
\stopitemize

این‌جا به طرز عجیبی (همان‌طور که در مستند \type{usage-01} نیز مشکل داشتیم)
از قلاب اشتباه استفاده می‌شود.  یک راه درمان موضعی‌اش بهره گرفتن از قلاب باز در
تنظیم محیط شمارشی است:
...
برای تکمیل کردن این بخش بد نیست ترکیب زیر را نیز به نمایش بگذاریم.

\startitemize[عددی,columns,three][right=-,stopper=,itemalign=left,distance=1ex]
\getbuffer[itemlistshort]
\stopitemize

%% TODO: try nested itemizations and alignment.
%% TODO: define RTL symbols.
